...
//assigin typsecipt annotation type 
//for startGame we us HTMLCollentionOf to help match with the right class name
const snakeGameBoard : HTMLElement = document.querySelector("#gameboard")!;
const scoreBoard : HTMLElement = document.querySelector("#score")!;
const startGame: HTMLElement | null = document.querySelector("#startGame"); 

//?is this a coordinate?
//create a type Position for the positions of the snake, the input and the foodbait
interface Position { x: number, y: number} ;

let snakePosition: Position[]  = [{ x: 5, y: 6 }]; //snake array
let inputDirection: Position = { x: 0, y: 0 };
let speed : number = 10;
let score : number = 0;
let lastPaintTime : number = 0;
let foodBait: Position = { x: 10, y: 10 };

//setup game function, the main function
//create
function main(startGame: any ): void {
...
function didSnakeCollide(snake: any): any {
...
function gamePlay() {
    ...
    snakeGameBoard.innerHTML = "";
        snakePosition.forEach((e: any, index: any) => {
        let snakeElement: HTMLElement = document.createElement("div")!;
    ...
    let foodElement: HTMLElement = document.createElement("div")!;
    foodElement.style.gridRowStart = foodBait.y.toString();
    foodElement.style.gridColumnStart = foodBait.x.toString();
...
//set our duration to a type of number, and the display to a HTMLELement
//as well as our timer to a type number so that the timer can start
function timeLeft(duration: number, display: HTMLElement): void{
...
function startTimer(): void {
    let oneMinute = 60,
    display: HTMLElement = document.querySelector("#time-left")!;
